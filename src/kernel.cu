...
__device__ glm::vec3 computeVelocityChange(int N, int iSelf,
    ...
    for (int i = 0; i < N; ++i) {
        ...
        float dist = glm::distance2(bpos, npos);
        if (dist < rule1Distance * rule1Distance) {
            perceived_center += npos;
            num_rule1_nbrs++;
        }
...
__global__ void kernComputeIndices(int N, int gridResolution, glm::vec3 gridMin,
    ...
    glm::vec3 off = pos[index] - gridMin;
    glm::ivec3 grid_idx = (glm::ivec3)(off * inverseCellWidth);

    gridIndices[index] =
        gridIndex3Dto1D(grid_idx.x, grid_idx.y, grid_idx.z, gridResolution);
    indices[index] = index;
}

__global__ void kernGatherPosVel(int N, glm::vec3 *pos1, glm::vec3 *pos2,
                                 glm::vec3 *vel1, glm::vec3 *vel2,
                                 int *indices) {
    int index = (blockIdx.x * blockDim.x) + threadIdx.x;
    if (index >= N) {
    ...
    pos2[index] = pos1[src_idx];
    vel2[index] = vel1[src_idx];
}
...
__device__ glm::vec3 computeVelocityChangeNeighborSearch(
    ...
    int bgrid_id =
        gridIndex3Dto1D(bgrid_idx.x, bgrid_idx.y, bgrid_idx.z, gridResolution);
    for (int z = -1; z <= 1; ++z) {
        for (int y = -1; y <= 1; ++y) {
            for (int x = -1; x <= 1; ++x) {
                ...
                int end_idx = gridCellEndIndices[ngrid_id];
...
__global__ void kernUpdateVelNeighborSearchCoherent(
    ...
    // indirection. This should expect gridCellStartIndices and
    // gridCellEndIndices to refer directly to pos and vel1.
    // - Identify the grid cell that this particle is in
    // - Identify which cells may contain neighbors. This isn't always 8.
    // - For each cell, read the start/end indices in the boid pointer array.
    //   DIFFERENCE: For best results, consider what order the cells should be
    //   checked in to maximize the memory benefits of reordering the boids
    //   data.
    // - Access each boid in the cell and compute velocity change from
    //   the boids rules, if this boid is within the neighborhood distance.
    // - Clamp the speed change before putting the new speed in vel2
...
void Boids::stepSimulationCoherentGrid(float dt) {
    ...
    thrust::sort_by_key(dev_thrust_particleGridIndices,
                        dev_thrust_particleGridIndices + numObjects,
                        dev_thrust_particleArrayIndices);
    kernGatherPosVel<<<fullBlocksPerGrid, blockSize>>>(
        numObjects, dev_pos, dev_pos2, dev_vel1, dev_vel2,
...
void Boids::unitTest() {
    ...
    thrust::sort_by_key(dev_thrust_keys, dev_thrust_keys + N,
                        dev_thrust_values);
